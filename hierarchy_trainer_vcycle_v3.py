"""
hierarchy_trainer_vcycle_v3.py - Multigrid V-cycle Trainer with Enhanced 2D Projection-based Hierarchy Consistency Loss
...
v3 (Current):
    ...
    - **Hybrid Weight System**: 
      - Covariance loss: Uses opacity × area based weights (v2 style, no_grad)
      - Color loss: Uses depth-based transmittence weights for image-space contribution
    - **Enhanced Position/Size Loss**: 
      - Children must stay inside parent gaussian (relu(dist - clamp(parent_size - child_size, min=0)))
      - Penalizes oversized children (relu(child_size - parent_size))
    - **Depth Consistency Loss**: Enforces parent and children to have similar depths (0.25 weight)
    - **Opacity Falloff**: Uses Browser paper formula (falloff = sum(weights) / sqrt(det(expected_covar)))
...
v2 (Previous):
    - **2D Projection-based Consistency**: Uses rasterization projection data (means2d, colors, conics, opacities)
    - **2D Covariance Weights**: Uses 2D covariance determinants (from conics) for aggregation weights
    - **Weighted Average Loss**: Expected means/covars computed as weighted average, compared with parent
    - **Browser Falloff**: Uses Browser paper falloff formula for opacity
...
- Spatial constraints: Children must stay within parent bounds, preventing oversized children
- Depth-aware weighting: Uses depth information for better image-space contribution modeling
...
- Weight computation:
  - Covariance: opacity × area based weights (no_grad, v2 style)
  - Color: depth-based transmittence (exp(-beta * depth) / sum(exp(-beta * depths)))
- Loss terms:
  - size_loss: inside_loss + oversize_loss (spatial constraints)
  - covar_loss: normalized Frobenius norm (expected vs parent covariance)
  - opacity_loss: Browser falloff formula
  - color_loss: premultiplied color with depth-based transmittence
  - depth_loss: MSE between parent and children depths (0.25 weight)
"""
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.result_dir is None:
            ...
            result_base = f"./results/hierarchy_trainer_vcycle_v3/{dataset_name}_{settings_str}"
...
def perform_vcycle(
    ...
    coarsest_level = max(1, min(coarsest_level, max_level))  # Clamp to [1, max_level]
        ...
        # in hierarchy_trainer_vcycle_v3.py, hooks are never registered.
...
class Runner:
    ...
    def _update_grad_scaling_hooks(self):
        ...
        For hierarchy_trainer_vcycle_v3.py: densification is disabled, so this is never called.
    ...
    def _extract_projection_data(self, info: Dict) -> Optional[Dict]:
        ...
        if gaussian_ids is not None:
            ...
            if depths is not None:
                sums_depths = torch.zeros(num_visible, device=device, dtype=depths.dtype)
                sums_depths.scatter_add_(0, gaussian_ids, depths)
                depths = sums_depths / counts.clamp_min(1)
    ...
    def _compute_hierarchy_consistency_loss(
        ...
        device = self.device
        empty_losses_dict = {
            "inside": torch.tensor(0.0, device=device),
            "oversize": torch.tensor(0.0, device=device),
        ...
        child_depths = children_proj.get("depths", None)
        if child_depths is not None:
            child_depths = child_depths[child_local]
        parent_depths = parent_proj.get("depths", None)

        # Apply detach based on direction
        # - downwards: parents are regularized, detach children
        # - upwards: children are regularized, detach parents
        if direction == "downwards":
            child_means2d = child_means2d.detach()
            child_covars2d = child_covars2d.detach()
            child_det_inv = child_det_inv.detach()
            child_opacities = child_opacities.detach()
            child_colors = child_colors.detach()
            if child_depths is not None:
                child_depths = child_depths.detach()
        else:  # upwards
            parent_means2d = parent_means2d.detach()
            parent_covars2d = parent_covars2d.detach()
            parent_opacities = parent_opacities.detach()
            parent_colors = parent_colors.detach()
            if parent_depths is not None:
                parent_depths = parent_depths.detach()

        if profile_detail:
            torch.cuda.synchronize()
            t_loss_start = time.time()

        # Position/covariance: children should stay inside the parent gaussian
        parent_means_per_child = parent_means2d[parent_local]
        parent_covars_per_child = parent_covars2d[parent_local]
        dvec = child_means2d - parent_means_per_child  # [K, 2]
        dist = torch.linalg.norm(dvec, dim=-1)  # [K,]
        u = dvec / (dist.unsqueeze(-1) + 1e-6)  # [K, 2]
        parent_extent = torch.sqrt(
            torch.einsum("bi,bij,bj->b", u, parent_covars_per_child, u).clamp_min(1e-12)
        )
        child_extent = torch.sqrt(
            torch.einsum("bi,bij,bj->b", u, child_covars2d, u).clamp_min(1e-12)
        )
        inside_margin = torch.clamp(parent_extent - child_extent, min=0.0)
        inside_loss = F.relu(dist - inside_margin).mean()
        oversize_loss = F.relu(child_extent - parent_extent).mean()
        size_loss = inside_loss + oversize_loss

        # Compute opacity * area based weights for covariance calculation (v2 style)
        ...
        with torch.no_grad():
            ...
            weights = unnormalized_weights / (parent_weight_sums[parent_local] + 1e-20)

        # Compute expected means2d for covariance calculation (using opacity * area weights)
        ...
        # Compute expected covars2d (include mean difference, using opacity * area weights)
        expected_means_per_child = expected_means2d[parent_local]
        mean_diffs = child_means2d - expected_means_per_child  # [K, 2]
        ...
        # Compute opacity falloff using Browser paper formula
        ...
        expected_opacities_linear_falloff = falloff / (falloff + 1.0)

        # Color/opacity: approximate transmittance and match premultiplied color (using depth-based weights)
        color_dim = child_colors.shape[-1]
        if child_depths is None:
            depth_scores = torch.ones_like(child_opacities)
        else:
            beta = 2.0
            depth_scores = torch.exp(-beta * child_depths)
        depth_scores = depth_scores.clamp_min(1e-12)

        depth_score_sums = torch.zeros(num_parents, device=device, dtype=depth_scores.dtype)
        depth_score_sums.scatter_add_(0, parent_local, depth_scores)
        approx_transmittence = depth_scores / (depth_score_sums[parent_local] + 1e-20)

        child_premul = child_colors * child_opacities.unsqueeze(-1)
        expected_premul = torch.zeros(
            num_parents, color_dim, device=device, dtype=child_colors.dtype
        )
        expected_premul.scatter_add_(
            0,
            parent_local.unsqueeze(-1).expand(-1, color_dim),
            child_premul * approx_transmittence.unsqueeze(-1),
        )
        ...
        parent_premul = parent_premul[valid_parent_mask]
        expected_opacities_linear_falloff = expected_opacities_linear_falloff[valid_parent_mask]
        ...
        parent_means2d_valid = parent_means2d[valid_parent_mask]
        parent_covars2d_valid = parent_covars2d[valid_parent_mask]

        # Apply detach based on direction
        # - downwards: parents are regularized, detach expected (from children)
        # - upwards: children are regularized, detach parent
        if direction == "downwards":
            expected_means2d = expected_means2d.detach()
            expected_covars2d = expected_covars2d.detach()
            expected_premul = expected_premul.detach()
            expected_opacities_linear_falloff = expected_opacities_linear_falloff.detach()
        else:  # upwards
            parent_means2d_valid = parent_means2d_valid.detach()
            parent_covars2d_valid = parent_covars2d_valid.detach()
            parent_premul = parent_premul.detach()
            parent_opacities_linear = parent_opacities_linear.detach()

        opacity_loss = F.mse_loss(expected_opacities_linear_falloff, parent_opacities_linear)
        color_loss = F.mse_loss(expected_premul, parent_premul)
        ...
        parent_covar_norm = parent_covars2d_valid.norm(dim=(1, 2))  # [num_valid_parents,]
        covar_diff_norm = (expected_covars2d - parent_covars2d_valid).norm(dim=(1, 2))  # [num_valid_parents,]
        ...
        if child_depths is not None and parent_depths is not None:
            parent_depths_per_child = parent_depths[parent_local]
            depth_loss = F.mse_loss(child_depths, parent_depths_per_child)
        else:
            depth_loss = torch.tensor(0.0, device=device)

        hierarchy_loss = size_loss + covar_loss + opacity_loss + color_loss + depth_loss * 0.25

        if profile_detail:
            torch.cuda.synchronize()
            t9 = time.time()
            timings["loss_compute"] = (t9 - t_loss_start) * 1000
    ...
    def _train_step(
        ...
        # For hierarchy_trainer_vcycle_v3.py, projection data (info_current, info_other) is passed from _train_step.
        losses_dict = {}
        if cfg.hierarchy_consistency_lambda > 0.0 and direction is not None:
            ...
            if step % 10 == 0:  # Print every 10 steps
                ...
                if cfg.hierarchy_consistency_lambda > 0.0:
                    ...
                    print(f"    - inside_loss: {losses_dict.get('inside', torch.tensor(0.0)).item():.6f}")
                    print(f"    - oversize_loss: {losses_dict.get('oversize', torch.tensor(0.0)).item():.6f}")
                    print(f"    - size_loss: {losses_dict.get('size', torch.tensor(0.0)).item():.6f}")
...
if __name__ == "__main__":
    ...
    ```bash
    # Single GPU training
    CUDA_VISIBLE_DEVICES=9 python hierarchy_trainer_vcycle_v3.py --dataset_type nerf --data_dir /path/to/data

    # Distributed training on 4 GPUs: Effectively 4x batch size so run 4x less steps.
    CUDA_VISIBLE_DEVICES=0,1,2,3 python hierarchy_trainer_vcycle_v3.py --dataset_type nerf --data_dir /path/to/data --steps_scaler 0.25
