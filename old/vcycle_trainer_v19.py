"""
vcycle_trainer_v19.py - Multigrid V-cycle Trainer with Render-based Restriction/Prolongation

VERSION HISTORY AND KEY DIFFERENCES:

v19 (Current):
    - **Parent Weight-based Parameter Sharing**: Added learnable `parent_weight` parameter (1D) for each Gaussian
      - Means: Uses existing residual-based approach (no weight)
      - Other parameters (scales, quats, opacities, sh0, shN): 
        `parent_parameter * (parent_weight / sum(all_children_parent_weights)) + child_residual`
      - Weight is constrained to be positive using `elu+1` activation in `get_splats`
      - Learning rate: 1e-3 (default)
    - **Updated Operations**: All densification operations (duplicate, split, clone_hierarchy_block, 
      _add_zero_parameter_children) now handle `parent_weight` parameter
    - **Updated Imports**: Uses `ops_mg_v7.py` and `multigrid_gaussians_v9.py`
...
from gsplat.strategy.multigrid_v14 import MultigridStrategy
from gsplat_viewer import GsplatViewer, GsplatRenderTabState
from nerfview import CameraState, RenderTabState, apply_float_colormap
from multigrid_gaussians_v9 import MultigridGaussians, quaternion_multiply
...
class Config:
    ...
    max_children_per_parent: int = 15  # Maximum number of children a parent can have
    ...
    def __post_init__(self):
        ...
        if self.result_dir is None:
            ...
            self.result_dir = f"./results/vcycle_trainer_v19_{self.cycle_type}/{dataset_name}_{settings_str}_v1"
        ...
        if self.pause_refine_after_level_increase is None:
            if self.cycle_type == "vcycle":
                self.pause_refine_after_level_increase = 0
            elif self.cycle_type == "inv_fcycle":
                self.pause_refine_after_level_increase = 0
            else:
                raise ValueError(f"Unknown cycle_type: {self.cycle_type}")
...
def vcycle_recursive(
    ...
    if level > coarsest_level:
        ...
        for data in down_smoothing_views[:restriction_steps]:
            ...
            with torch.no_grad():
                ...
                level_diff = image_multigrid_max_level - coarse_level
            ...
            restriction_loss = 0.25 * relation_loss + 0.75 * gt_loss
            ...
            torch.cuda.empty_cache()
        # Metric after restriction
        log_level_metric(tag=f"DownPostRestrict L{target_level}", print_metric=False)
    ...
    if False and level > coarsest_level:
        ...
        restriction_steps = max(1, current_smoothing_steps // 2)
        prolongation_steps = max(1, restriction_steps // 2)
        for data in down_smoothing_views[:prolongation_steps]:
...
class Runner:
    ...
    def __init__(
        ...
        if isinstance(self.cfg.strategy, MultigridStrategy):
            ...
            self.cfg.strategy.grad_scale_factor = self.cfg.grad_scale_factor
    ...
    def _compute_current_max_level(self, step: int) -> Optional[int]:
        ...
        start_level = 1 if self.cfg.init_level1_ratio >= 1.0 else 2
        start_level = min(start_level, final_max_level)
    ...
    def _train_step(
        ...
        if False and target_level < max_level_to_use:
            ...
            with torch.no_grad():
                ...
                # target_level + 1 has (1/level_resolution_factor)x higher resolution than target_level
                finer_level_diff = image_multigrid_max_level - (target_level + 1)
                finer_downsample = (1.0 / cfg.level_resolution_factor) ** finer_level_diff
                finer_downsample = max(1, int(finer_downsample))
        ...
        key_for_gradient = getattr(self.cfg.strategy, "key_for_gradient", None)
        info_for_strategy = {
            "width": info["width"],
            "height": info["height"],
        ...
        if "gaussian_ids" in info and info["gaussian_ids"] is not None:
            info_for_strategy["gaussian_ids"] = info["gaussian_ids"]
        if "radii" in info and info["radii"] is not None:
            info_for_strategy["radii"] = info["radii"].detach()
        if key_for_gradient is not None and key_for_gradient in info:
            grad_tensor = info[key_for_gradient].grad
            if grad_tensor is None:
                grad_value = torch.zeros_like(info[key_for_gradient])
            else:
                grad_value = grad_tensor.detach()
            grad_proxy = SimpleNamespace(grad=grad_value)
            if getattr(self.cfg.strategy, "absgrad", False):
                if hasattr(info[key_for_gradient], "absgrad") and info[key_for_gradient].absgrad is not None:
                    grad_proxy.absgrad = info[key_for_gradient].absgrad.detach()
                else:
                    grad_proxy.absgrad = grad_value.abs()
            info_for_strategy[key_for_gradient] = grad_proxy

        # Turn Gradients into Sparse Tensor before running optimizer
        if cfg.sparse_grad:
            assert cfg.packed, "Sparse gradients only work with packed mode."
            visible_indices = torch.where(info_for_strategy["visible_mask"])[0]
            gaussian_ids = info_for_strategy["gaussian_ids"]
            if len(visible_indices) > 0 and len(gaussian_ids) > 0:
                full_ids = visible_indices[gaussian_ids]
