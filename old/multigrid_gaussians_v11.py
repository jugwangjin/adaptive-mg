...
def child_to_parent_delta(
    ...
    - scales: parent_delta = child_delta (no change, but -CHILD_SCALE_LOG is handled separately)
    ...
    if param_name == "means":
        ...
        # scales: parent_delta = child_delta (no change needed, -CHILD_SCALE_LOG is constant)
        return child_delta
...
class MultigridGaussians:
    ...
    def __init__(
        ...
        # Keep parameters and rendering on the same device to avoid CPU offload.
        self.render_device = self.param_device
        ...
        def _compute_scales(points: Tensor) -> Tensor:
            n_points = len(points)
            if n_points == 0:
                return torch.empty(0, 3, device=device)
            if n_points == 1:
                dist_avg = torch.ones(1, device=device)
            else:
                k = min(4, n_points)
                dists = knn(points, k)
                if k <= 1:
                    dist_avg = torch.ones(n_points, device=device)
                else:
                    dist_avg = torch.sqrt((dists[:, 1:] ** 2).mean(dim=-1))
            return torch.log(dist_avg * init_scale).unsqueeze(-1).repeat(1, 3)

        if len(points_all) > 0:
            scales_all = _compute_scales(points_all)  # [n_all, 3]
            if len(scales_all) > 0:
            ...
            scales_level1 = _compute_scales(points_level1)  # [n_level1, 3]
            if len(scales_level1) > 0:
            ...
            scales_level2 = _compute_scales(points_level2)  # [n_level2, 3]
            if len(scales_level2) > 0:
        ...
        start_level1_only = (n_level2 == 0)
        if max_level is not None and max_level > 1 and not start_level1_only:
    ...
    def restrict_parameters(
        ...
        - Handles special cases: means (scaling), scales (-CHILD_SCALE_LOG), quats (quaternion multiplication)
        - Note: parent_level = child_level - 1 (always), so we don't need parent_level parameter
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if "gaussian_ids" in info and info["gaussian_ids"] is not None:
            info["gaussian_ids"] = info["gaussian_ids"].to(device)
        if "radii" in info and info["radii"] is not None:
            info["radii"] = info["radii"].to(device)
