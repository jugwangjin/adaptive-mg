...
class MultigridGaussians:
    ...
    def __init__(
        ...
        self.cfg = cfg
        self.param_device = torch.device(device)
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if camtoworlds.device != render_device:
            camtoworlds = camtoworlds.to(render_device)
        if Ks.device != render_device:
            Ks = Ks.to(render_device)
        if masks is not None and masks.device != render_device:
            masks = masks.to(render_device)
        ...
        if len(visible_indices) == 0:
            ...
            render_colors = torch.zeros((C, height, width, 3), device=render_device)
            render_alphas = torch.zeros((C, height, width, 1), device=render_device)
            info = {
                "gaussian_ids": torch.empty((0,), dtype=torch.long, device=param_device),
                "radii": torch.empty((C, 0, 2), device=param_device),
                "means2d": torch.empty((C, 0, 2), device=render_device),  # Required for step_pre_backward
                "width": width,
                "height": height,
                "n_cameras": C,
                "visible_mask": torch.zeros(len(self.levels), dtype=torch.bool, device=param_device),
        ...
        visible_means_cpu = visible_splats["means"][visible_indices]  # [N_visible, 3]
        visible_quats_cpu = visible_splats["quats"][visible_indices]  # [N_visible, 4]
        visible_scales_cpu = visible_splats["scales"][visible_indices]  # [N_visible, 3]
        visible_opacities_cpu = visible_splats["opacities"][visible_indices]  # [N_visible,]
        ...
        if image_ids is not None:
            image_ids = image_ids.to(render_device)
        try:
            if self.cfg.app_opt:
                visible_features_cpu = visible_splats["features"][visible_indices]
                visible_colors_cpu = visible_splats["colors"][visible_indices]
                visible_features = visible_features_cpu.to(render_device)
                visible_colors = visible_colors_cpu.to(render_device)
                del visible_features_cpu, visible_colors_cpu
            else:
                visible_sh0_cpu = visible_splats["sh0"][visible_indices]
                visible_shN_cpu = visible_splats["shN"][visible_indices]
                visible_sh0 = visible_sh0_cpu.to(render_device)
                visible_shN = visible_shN_cpu.to(render_device)
                del visible_sh0_cpu, visible_shN_cpu
        except:
            visible_sh0_cpu = visible_splats["sh0"][visible_indices]
            visible_shN_cpu = visible_splats["shN"][visible_indices]
            visible_sh0 = visible_sh0_cpu.to(render_device)
        ...
        means = visible_means_cpu.to(render_device)  # [N_visible, 3]
        quats = visible_quats_cpu.to(render_device)  # [N_visible, 4] (rasterization normalizes internally)
        scales = torch.exp(visible_scales_cpu.to(render_device))  # [N_visible, 3]
        opacities = torch.sigmoid(visible_opacities_cpu.to(render_device))  # [N_visible,]
        ...
        # Free intermediate tensors
        del visible_means_cpu, visible_quats_cpu, visible_scales_cpu, visible_opacities_cpu
        ...
        full_visible_mask = torch.zeros(N_total, dtype=torch.bool, device=param_device)
        ...
        if "gaussian_ids" in info and info["gaussian_ids"] is not None:
            info["gaussian_ids"] = info["gaussian_ids"].to(param_device)
        if "radii" in info and info["radii"] is not None:
            info["radii"] = info["radii"].to(param_device)
