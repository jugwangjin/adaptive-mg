...
class Config:
    ...
    # Coarse-to-fine: maximum level (determines maximum resolution)
    max_level: int = 4
...
class Runner:
    ...
    def render_at_level(
        ...
        """
        Render at a specific resolution level (coarse-to-fine).
        Only adjusts image resolution, not Gaussian structure.
        
        Args:
            target_level: Resolution level (1 = coarsest, max_level = finest)
        ...
        # Render at target resolution (no level parameter for simple trainer)
        ...
        # Verify that rendered resolution matches target resolution
        # rendered_height, rendered_width = rendered_colors.shape[1:3]
        # if rendered_height != height or rendered_width != width:
        #     print(f"WARNING: Resolution mismatch in render_at_level! "
        #           f"Expected: {height}x{width}, Got: {rendered_height}x{rendered_width}, "
        #           f"target_level={target_level}, downsample_factor={downsample_factor}")
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            num_train_rays_per_step = (
                pixels_gt.shape[0] * pixels_gt.shape[1] * pixels_gt.shape[2]
            )
            ...
            # Render at target_level using render_at_level
            ...
            # Use downsampled pixels for loss calculation
            pixels = pixels_downsampled
            height, width = pixels.shape[1:3]
            
            # Extract depths if available
            if cfg.depth_loss:
                # Depths are not returned from render_at_level, need to render separately or extract from info
                # For now, we'll need to handle this differently
                depths = None  # Will be handled in depth loss calculation if needed
            else:
                depths = None
            ...
            if cfg.depth_loss:
                # For depth loss, we need to render at full resolution or handle differently
                # For now, skip depth loss in coarse-to-fine mode
                # TODO: Implement depth loss for coarse-to-fine training
            ...
            desc = f"loss={loss.item():.3f}| level={target_level}| sh degree={sh_degree_to_use}| step={step}/{max_steps}"
            if cfg.depth_loss:
            ...
            if cfg.sparse_grad:
                ...
                for k in self.splats.keys():
                    ...
                    self.splats[k].grad = torch.sparse_coo_tensor(
                        ...
                        is_coalesced=camtoworlds.shape[0] == 1,
                    )
    ...
    def save_visualization(self, step: int):
        ...
        for cam_idx in self.viz_camera_indices:
            ...
            pixels_gt_for_viz = pixels_downsampled[0]  # [H, W, 3] (downsampled GT)
            
            # Convert to numpy and stack: [H, W, 3] for GT, [H, W, 3] for render
            pixels_gt_np = pixels_gt_for_viz.cpu().numpy()
