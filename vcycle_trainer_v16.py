...
from gsplat.strategy.multigrid_v12 import MultigridStrategy
from gsplat_viewer import GsplatViewer, GsplatRenderTabState
from nerfview import CameraState, RenderTabState, apply_float_colormap
from multigrid_gaussians_v7 import MultigridGaussians, quaternion_multiply
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.result_dir is None:
            ...
            self.result_dir = f"./results/vcycle_trainer_v15_{self.cycle_type}/{dataset_name}_{settings_str}_v1"
...
class Runner:
    ...
    def __init__(
        ...
        self.max_level_reached_step = None
        if self.final_max_level is not None and self.current_max_level == self.final_max_level:
            self.max_level_reached_step = 0
    ...
    def _maybe_update_max_level(self, step: int, cycle_idx: int) -> None:
        ...
        if (
            self.final_max_level is not None
            and target_max_level == self.final_max_level
            and self.max_level_reached_step is None
        ):
            self.max_level_reached_step = step
    ...
    def _compute_current_coarsest_level(
        self, step: int, current_max_level: int
    ) -> int:
        """Diamond schedule: widen (max level grows), then narrow (coarsest grows)."""
        interval = self.cfg.level_increase_interval
        if interval <= 0:
            return 1
        if self.final_max_level is None:
            return 1
        if current_max_level < self.final_max_level:
            return 1
        reach_step = self.max_level_reached_step
        if reach_step is None:
            reach_step = step
            self.max_level_reached_step = step
        steps_since = max(0, step - reach_step)
        increase_steps = steps_since // interval
        return max(1, min(current_max_level, 1 + increase_steps))
    ...
    def train(self):
        ...
        while current_step < max_steps:
            ...
            if cfg.cycle_type == "vcycle":
                ...
                cycle_coarsest_level = self._compute_current_coarsest_level(
                    current_step, cycle_max_level
                )
