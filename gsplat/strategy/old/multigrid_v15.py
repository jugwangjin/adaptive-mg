...
class MultigridStrategy(Strategy):
    ...
    pause_refine_after_level_increase: int = 3  # Pause refining for N V-cycles after max level increase
    verbose: bool = False
    absgrad: bool = False  # Use absolute gradients for GS splitting
    ...
    def update_max_level(
        ...
        """Update max_level and expand hierarchy if a new level is enabled."""
        if max_level is None:
            return False

        max_level = int(max_level)
        current_max_level = state.get("max_level", self.max_level)
        ...
        if "multigrid_gaussians" in state and state["multigrid_gaussians"] is not None:
            state["multigrid_gaussians"].max_level = max_level
        ...
        _add_zero_parameter_children(
            ...
            state=state,
            levels=levels,
            parent_indices_tensor=state["parent_indices"],
    ...
    def post_cycle(
        ...
        # Check pause after level increase (cycle-based)
        if self.pause_refine_after_level_increase > 0:
            if "last_level_increase_cycle" in state:
                if cycle - state["last_level_increase_cycle"] < self.pause_refine_after_level_increase:
                    return
        ...
        # Grow gaussians once using gradients from current max level
        ...
        # Densify only at current max level (use grads from that render level).
        levels = state["levels"]
        N = len(levels)
        densify_level = state.get("max_level", None)
        if densify_level is None:
            densify_level = int(levels.max().item()) if N > 0 else 1
        densify_level = int(densify_level)
        ...
        N_final = len(levels)
        if N_final > 0:
            invalid_parent_mask = (parent_indices < -1) | (parent_indices >= N_final)
            if invalid_parent_mask.any():
                parent_indices = parent_indices.clone()
                parent_indices[invalid_parent_mask] = -1
                state["parent_indices"] = parent_indices
                level_indices_new = {
                    level_val.item(): torch.where(levels == level_val)[0].tolist()
                    for level_val in levels.unique()
                }
        ...
        if self.verbose:
            ...
            densify_stats = state.get("last_densify_level_stats")
            if densify_stats is not None:
                densify_level_stats = []
                for level in sorted(level_indices.keys()):
                    clone_count = densify_stats["clone"].get(level, 0)
                    duplicate_count = densify_stats["duplicate"].get(level, 0)
                    split_count = densify_stats["split"].get(level, 0)
                    densify_level_stats.append(
                        f"L{level}: clone={clone_count}, dup={duplicate_count}, split={split_count}"
                    )
                densify_level_stats_str = ", ".join(densify_level_stats)
                print(f"  Densify level-wise: {densify_level_stats_str}")
    ...
    def _grow_gs(
        ...
        # Aggregated gradients are already averaged per level, so skip re-averaging.
        if use_aggregated:
            grads = grad2d  # [N,]
        else:
        ...
        if self.use_gradient_inheritance and len(level_indices) > 0:
            ...
            for level in range(coarsest_level, finest_level + 1):
                ...
                if visible_mask.any():
                    ...
                    # Inherited gradient formula: (parent_grad / parent_radii * child_radii) / num_children * gradient_inheritance_factor
                    inherited_grad = ((p_grads / p_radii) * c_radii) / p_n_child * self.gradient_inheritance_factor
        ...
        # Apply level-wise gradient scaling to thresholds (match trainer scaling)
        if self.grad_scale_factor != 1.0 and N > 0:
            actual_max_level = int(levels.max().item()) if N > 0 else int(max_level or 1)
            level_diffs = actual_max_level - levels.float()
            level_scales = torch.pow(
                torch.tensor(self.grad_scale_factor, device=device, dtype=level_diffs.dtype),
                level_diffs,
            )
            level_thresholds = level_thresholds * level_scales
        ...
        # Densification only at current max level.
        densify_level = state.get("max_level", None)
        if densify_level is None:
            densify_level = int(levels.max().item()) if N > 0 else 1
        densify_level = int(densify_level)
        is_grad_high = is_grad_high & (levels == densify_level)
        ...
        # Clone is preferred over densify for clone-related nodes. Propagate until we find
        # a parent that can accept a new child; keep roots as clone targets.
        clone_parents_pending = clone_parents.clone()
        clone_parents_pending[is_root] = False
        changed = True
        while changed:
            changed = False
            clone_parents_candidates = torch.where(
                clone_parents_pending & ~already_densified & ~is_root
            )[0]
            ...
            # Parents that can accept a new child: stop propagation, keep as clone targets.
            can_clone_here = ~clone_parents_parents_full
            if can_clone_here.any():
                clone_parents_pending[clone_parents_candidates[can_clone_here]] = False
                changed = True

            # Must propagate further upward
            must_propagate = clone_parents_parents_full
            if must_propagate.any():
                propagate_children = clone_parents_candidates[must_propagate]
                propagate_parents = clone_parents_parents[must_propagate]
                # Filter valid parents and not yet densified
                valid_mask = (
                    (propagate_parents >= 0)
                    & (propagate_parents < N)
                    & ~already_densified[propagate_parents]
                )
                if valid_mask.any():
                    clone_children_mask[propagate_children[valid_mask]] = True
                    clone_parents[propagate_parents[valid_mask]] = True
                    clone_parents_pending[propagate_parents[valid_mask]] = True
                    clone_parents_pending[propagate_children[valid_mask]] = False
                    changed = True

        # Densify only nodes unrelated to clone.
        if clone_parents.any() or clone_children_mask.any():
            should_densify[clone_parents] = False
            should_densify[clone_children_mask] = False
        ...
        cloned_source_indices = None  # Original indices cloned (for stats)
        levels_pre_clone = levels
        ...
        if len(clone_parents_indices) > 0:
            ...
            cloned_indices_for_densify, cloned_source_indices = clone_hierarchy_block(
                params=params,
                optimizers=optimizers,
                state=state,
        ...
        if self.verbose:
            ...
            level_keys = sorted(level_indices.keys())
            max_level_value = max(level_keys) if len(level_keys) > 0 else int(levels.max().item()) if current_N > 0 else 1

            def count_by_level(mask: Optional[Tensor] = None, indices: Optional[Tensor] = None, levels_tensor: Optional[Tensor] = None) -> Dict[int, int]:
                if levels_tensor is None:
                    levels_tensor = levels
                if indices is not None and indices.numel() > 0:
                    level_vals = levels_tensor[indices]
                elif mask is not None and mask.any():
                    level_vals = levels_tensor[mask]
                else:
                    return {level: 0 for level in level_keys}
                counts = torch.bincount(level_vals, minlength=max_level_value + 1)
                return {level: int(counts[level].item()) for level in level_keys}

            clone_counts = (
                count_by_level(indices=cloned_source_indices, levels_tensor=levels_pre_clone)
                if cloned_source_indices is not None
                else {level: 0 for level in level_keys}
            )
            duplicate_counts = count_by_level(mask=is_duplicate)
            split_counts = count_by_level(mask=is_split)

            state["last_densify_level_stats"] = {
                "clone": clone_counts,
    ...
    def _prune_gs(
        ...
        """Prune gaussians based on opacity and scale with hierarchical safeguards.
        
        Rules:
        - Root nodes are never pruned.
        - A child subtree is pruned if it is fully prunable and the parent keeps other children.
        - If all children are prunable, they are only pruned when their parent is prunable
          (otherwise they are kept to avoid parent popping).
        """
        # Get hierarchical structure
        levels = state["levels"]  # [N,]
        parent_indices = state["parent_indices"]  # [N,]
        ...
        # Compute prune eligibility for all nodes
        with torch.no_grad():
            ...
            actual_scales = torch.exp(actual_splats["scales"])
            del actual_splats
            ...
            # threshold_multiplier = 2.0 ** (max_level - levels.float())
            level_thresholds = self.prune_scale3d * state["scene_scale"]

            is_opacity_low = actual_opacities < self.prune_opa
            is_too_big = actual_scales.max(dim=-1).values > level_thresholds

            del actual_opacities, actual_scales, level_thresholds

        # Base prune candidates
        ...
        if valid_parent_mask.any():
            ...
            child_count = torch.zeros(N, dtype=torch.long, device=levels.device)

        # Compute subtree-prunable (all nodes in subtree satisfy to_prune)
        subtree_prunable = torch.zeros(N, dtype=torch.bool, device=levels.device)
        for level in range(finest_level, coarsest_level - 1, -1):
            level_mask = levels == level
            if not level_mask.any():
                continue
            prunable_child_count = torch.zeros(N, dtype=torch.long, device=levels.device)
            if valid_parent_mask.any():
                prunable_child_count.scatter_add_(
                    0,
                    parent_indices[valid_parent_mask],
                    subtree_prunable[valid_parent_mask].to(torch.long),
                )
            level_child_count = child_count[level_mask]
            level_prunable_child_count = prunable_child_count[level_mask]
            level_all_children_prunable = (level_child_count > 0) & (
                level_prunable_child_count == level_child_count
            )
            subtree_prunable[level_mask] = to_prune[level_mask] & (
                level_all_children_prunable | (level_child_count == 0)
            )

        # Decide final prune mask with hierarchical safeguards
        prunable_child_count = torch.zeros(N, dtype=torch.long, device=levels.device)
        if valid_parent_mask.any():
            prunable_child_count.scatter_add_(
                0,
        ...
        all_children_prunable = (child_count > 0) & (
            prunable_child_count == child_count
        )

        parent_all_children_prunable = torch.zeros(
            N, dtype=torch.bool, device=levels.device
        )
        if valid_parent_mask.any():
            parent_all_children_prunable[valid_parent_mask] = all_children_prunable[
                parent_indices[valid_parent_mask]
            ]
        ...
        prune_roots = (
            subtree_prunable
            & valid_parent_mask
            & ~parent_all_children_prunable
        )

        prune_mask = prune_roots.clone()
        for level in range(coarsest_level + 1, finest_level + 1):
            level_mask = levels == level
            if not level_mask.any():
                continue
            level_indices_tensor = torch.where(level_mask)[0]
            level_parents = parent_indices[level_indices_tensor]
            level_valid_parent_mask = (level_parents >= 0) & (level_parents < N)
            if level_valid_parent_mask.any():
                level_children = level_indices_tensor[level_valid_parent_mask]
                level_parents = level_parents[level_valid_parent_mask]
                prune_mask[level_children] = prune_mask[level_children] | prune_mask[
                    level_parents
                ]

        if prune_mask.any():
            n_pruned_level = prune_mask.sum().item()
            n_pruned_opacity_level = (prune_mask & is_opacity_low & ~is_too_big).sum().item()
            n_pruned_too_big_level = (prune_mask & is_too_big).sum().item()
        ...
        del to_prune, is_opacity_low, is_too_big, prune_mask
        torch.cuda.empty_cache()
