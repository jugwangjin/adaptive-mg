...
class MultigridStrategy(Strategy):
    ...
    def post_cycle(
        ...
        # Grow gaussians
        n_dupli_split, n_create_children = self._grow_gs(params, optimizers, state, step)
        
        # Update hierarchical structure after grow
        if "multigrid_gaussians" in state and state["multigrid_gaussians"] is not None:
            multigrid_gaussians = state["multigrid_gaussians"]
            state["levels"] = multigrid_gaussians.levels
            state["parent_indices"] = multigrid_gaussians.parent_indices
        ...
        # Reset running stats after densification
        if "grad2d" in state:
            state["grad2d"].zero_()
        if "count" in state:
            state["count"].zero_()
        ...
        if self.verbose:
            ...
            print(f"Cycle {cycle} (step {step}): Duplicated/Split: {n_dupli_split}, Created children: {n_create_children}, Pruned: {n_pruned}")
    ...
    def _update_state(
        ...
        if "count" not in state:
            state["count"] = torch.zeros(N, dtype=torch.float32, device=device)
        ...
        if packed:
            ...
            state["grad2d"].index_add_(0, full_gs_ids, grad2d)
            state["count"].index_add_(0, full_gs_ids, count)
        else:
    ...
    def _grow_gs(
        ...
        """
        count = state["count"]
        grad2d = state["grad2d"]  # Raw accumulated gradients
        ...
        # Handle parent duplicate/split signals
    ...
    def _prune_gs(
        ...
        for level in range(finest_level, coarsest_level - 1, -1):
            ...
            if level_prune_mask.any():
                ...
                del prune_mask, level_prune_indices, level_prune_mask
            
            # Free level-specific tensors after processing this level
            del to_prune, level_indices_tensor, num_child, is_leaf
            torch.cuda.empty_cache()
        ...
        if total_pruned > 0 and self.verbose:
            ...
            print(f"After pruning (step {step}, {total_pruned} total):")
