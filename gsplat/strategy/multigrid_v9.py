...
from .ops_mg_v3 import duplicate, remove_mg, reset_opa_mg, split
...
class MultigridStrategy(Strategy):
    ...
    def _grow_gs(
        ...
        Algorithm Overview:
        1. Validate and prepare gradients/counts from target_level
        ...
        if target_level not in state.get("count", {}) or target_level not in state.get("grad2d", {}):
        ...
        grad_color = self._ensure_tensor_size(
            state.get("grad_color", {}).get(target_level, torch.zeros_like(grad2d)),
            N, device, torch.float32
        )
        if "grad_color" not in state:
            state["grad_color"] = {}
        state["grad_color"][target_level] = grad_color
        ...
        color_grads = grad_color / count.clamp_min(1.0)  # [N,]
        ...
        if self.use_gradient_inheritance and len(level_indices) > 0:
            ...
            for level in range(coarsest_level, finest_level + 1):
                ...
                level_gaussians = torch.tensor(level_indices[level], dtype=torch.long, device=device)
                ...
                if visible_mask.any():
                    ...
                    p_color_grads = color_grads[parents[visible_mask]]
                    ...
                    inherited_color_grad = ((p_color_grads / p_radii) * c_radii) / p_n_child
                    
                    # Add to children
                    grads[children[visible_mask]] += inherited_grad
                    color_grads[children[visible_mask]] += inherited_color_grad
        ...
        # Check if gradients exceed thresholds (grad2d OR color)
        is_grad2d_high = grads > level_thresholds  # [N,]
        is_color_grad_high = color_grads > self.grow_color  # [N,]
        is_grad_high = is_grad2d_high | is_color_grad_high  # [N,] - integrated threshold
        ...
        with torch.no_grad():
            ...
            scale_thresholds = self.grow_scale3d * state["scene_scale"]
        ...
        # ========== Step 4: Determine densification candidates (coarsest to finest) ==========
        
        should_densify = torch.zeros(N, dtype=torch.bool, device=device)
        parent_signals = torch.zeros(N, dtype=torch.bool, device=device)
        ...
        for level in range(finest_level, coarsest_level - 1, -1):
            ...
            # Must signal parent
            must_signal = non_root_parents_full
            if must_signal.any():
                signal_parents = non_root_parents[must_signal]
                valid_parents = signal_parents[(signal_parents >= 0) & (signal_parents < N)]
                if len(valid_parents) > 0:
                    not_densified = ~already_densified[valid_parents]
                    parent_signals[valid_parents[not_densified]] = True
        
        # ========== Step 5: Recursively propagate parent signals upward ==========
        # If a parent receives a signal but cannot densify (its parent is full),
        # propagate the signal further upward until reaching root
        ...
        for level in range(finest_level, coarsest_level - 1, -1):
            ...
            signaled_mask = parent_signals[level_gaussians]
            if not signaled_mask.any():
                continue
            
            signaled = level_gaussians[signaled_mask]  # [K,]
            signaled_is_root = is_root[signaled]  # [K,]
            signaled_parents = parent_indices[signaled]  # [K,]
            signaled_parents_full = is_parent_full[signaled]  # [K,]
            
            # Root nodes: can always densify
            root_signaled = signaled[signaled_is_root]
            should_densify[root_signaled] = True
            parent_signals[root_signaled] = False
            
            # Non-root nodes: check if they can densify or need to signal further
            non_root_signaled = signaled[~signaled_is_root]
            if len(non_root_signaled) == 0:
                continue
            
            non_root_parents_full = signaled_parents_full[~signaled_is_root]
            non_root_parents = signaled_parents[~signaled_is_root]
            
            # Can densify directly
            can_densify = ~non_root_parents_full
            should_densify[non_root_signaled[can_densify]] = True
            parent_signals[non_root_signaled[can_densify]] = False
            
            # Must propagate signal further upward
            must_propagate = non_root_parents_full
            if must_propagate.any():
                propagate_parents = non_root_parents[must_propagate]
                valid_parents = propagate_parents[(propagate_parents >= 0) & (propagate_parents < N)]
                if len(valid_parents) > 0:
                    not_densified = ~already_densified[valid_parents]
                    parent_signals[valid_parents[not_densified]] = True
                # Remove current signal (replaced by parent signal)
                parent_signals[non_root_signaled[must_propagate]] = False
        
        # Merge signals into densification flags
        should_densify = should_densify | parent_signals
        
        # ========== Step 6: Determine duplicate vs split and execute ==========
        ...
        if self.verbose:
            print(f"  Densification: duplication(root={n_dupli_root}, non-root={n_dupli_non_root}), "
                  f"split(root={n_split_root}, non-root={n_split_non_root}), create_children={n_create_children}")
        ...
        if n_dupli_root > 0 or n_dupli_non_root > 0 or n_split_root > 0 or n_split_non_root > 0:
            ...
            for render_level in state.get("grad_color", {}):
                if len(state["grad_color"][render_level]) == N:
                    state["grad_color"][render_level][densified_indices] = 0
        ...
        if n_dupli_non_root > 0 or n_dupli_root > 0:
            ...
            # mask_root should be a subset of mask, indicating which gaussians in mask are roots
            mask_root_duplicate = is_duplicate_root & is_duplicate_combined if n_dupli_root > 0 else None
            duplicate(
                params=params,
                optimizers=optimizers,
                state=state,
                ...
                parent_indices=parent_indices,
                level_indices=level_indices,
                mask_root=mask_root_duplicate,
                max_level=max_level if n_dupli_root > 0 else None,
            )
        ...
        if n_split_non_root > 0 or n_split_root > 0:
            ...
            # mask_root should be a subset of mask, indicating which gaussians in mask are roots
            mask_root_split = is_split_root & is_split_combined if n_split_root > 0 else None
            split(
                params=params,
                optimizers=optimizers,
                state=state,
                ...
                mask_root=mask_root_split,
                max_level=max_level if n_split_root > 0 else None,
            )
        ...
        return total_dupli + total_split, n_create_children, already_densified
