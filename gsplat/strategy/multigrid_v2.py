...
class MultigridStrategy(Strategy):
    ...
    grad_threshold_decay_rate: float = 0.9  # Decay rate for gradient threshold per level: threshold = grow_grad2d * (decay_rate ** (level-1))
    ...
    def step_post_backward(
        ...
        if (
            ...
            # grow GSs
            n_dupli, n_split, n_create_children, n_root_dupli = self._grow_gs(params, optimizers, state, step, actual_splats=actual_splats)
            if self.verbose:
                ...
                print(
                    f"Step {step}: {n_dupli} GSs duplicated, {n_split} GSs split, {n_create_children} GSs created children, {n_root_dupli} GSs duplicated at root. "
    ...
    def _grow_gs(
        ...
        # If at max_level, check if tree is full (all nodes from root to leaf have max_children_per_parent)
        # If tree is full, duplicate root node instead of allowing infinite duplication
        at_max_level = (levels >= max_level) if max_level is not None else torch.zeros(N, dtype=torch.bool, device=device)
        
        # Compute tree fullness: check if all nodes from root to leaf have num_child >= max_children_per_parent
        # This is computed bottom-up: a node's subtree is "full" if it has max children AND all its children's subtrees are full
        is_node_full = (num_child >= self.max_children_per_parent)  # [N,] - node has max children
        
        # Max level leaf nodes: they cannot have children, so they are always considered "full" (cannot expand further)
        at_max_level = (levels >= max_level) if max_level is not None else torch.zeros(N, dtype=torch.bool, device=device)
        is_max_level_leaf = at_max_level  # [N,] - max level nodes without children
        
        # Initialize is_subtree_full: max level leaf nodes are always full (cannot expand)
        is_subtree_full = is_node_full.clone()  # [N,] - will be updated bottom-up
        is_subtree_full[is_max_level_leaf] = True  # Max level leaf nodes are always full
        
        # Bottom-up propagation: iterate from max_level down to level 1
        # A node's subtree is full if: (1) it has max children AND (2) all its children's subtrees are full
        if max_level is not None and max_level > 1:
            # Convert is_subtree_full to long integer for scatter operations
            is_subtree_full_int = is_subtree_full.long()  # [N,] - 1 if subtree is full, 0 otherwise
            
            for level in range(max_level, 1, -1):  # From max_level down to 2
                ...
                level_parent_indices = parent_indices[level_gaussian_indices]  # [M,]
                ...
                valid_children = level_gaussian_indices[valid_parent_mask]  # [K,]
                valid_parents = level_parent_indices[valid_parent_mask]  # [K,]
                
                # Use scatter_add to count how many children have full subtrees for each parent
                # num_children_full[i] = number of children of node i that have full subtrees
                num_children_full = torch.zeros(N, dtype=torch.long, device=device)
                num_children_full.scatter_add_(0, valid_parents, is_subtree_full_int[valid_children])
                
                # Parent's subtree is full if:
                # (1) it has max children (is_node_full) AND
                # (2) all its children's subtrees are full (num_children_full >= num_child)
                # Only update for parents that have children
                has_children_mask = num_child > 0
                all_children_full = num_children_full >= num_child
                is_subtree_full[has_children_mask] = is_node_full[has_children_mask] & all_children_full[has_children_mask]
                
                # Update integer version for next iteration
                is_subtree_full_int = is_subtree_full.long()
        
        # Identify root nodes (parent == -1)
        is_root = (parent_indices == -1)
        
        # At max_level: if tree is full, duplicate root node instead of allowing infinite duplication
        # Root node duplication: if root's subtree is full AND we're at max_level (some descendant is at max_level)
        root_nodes_to_duplicate = torch.zeros(N, dtype=torch.bool, device=device)
        # if max_level is not None:
            # Find root nodes whose subtrees are full (vectorized)
            # If root's subtree is full, allow root duplication
        root_nodes_to_duplicate = is_root & is_subtree_full
        
        # Only allow duplication at max_level if NOT a root node AND subtree is not full
        can_duplicate = can_duplicate | (at_max_level & has_parent & (~is_subtree_full))
        ...
        # Level 1: threshold = grow_grad2d * (decay_rate ** 0) = grow_grad2d
        # Level 2: threshold = grow_grad2d * (decay_rate ** 1) = grow_grad2d * decay_rate
        # Level 3: threshold = grow_grad2d * (decay_rate ** 2) = grow_grad2d * decay_rate^2
        # etc.
        ...
        # First, handle root node duplication if tree is full at max_level
        # Root nodes should be duplicated if their subtree is full (all descendants have max children)
        # This allows the tree to expand by creating new root nodes
        n_root_dupli = 0
        if root_nodes_to_duplicate.any():
            # Only duplicate root nodes with high gradient (same condition as other operations)
            root_dupli_mask = root_nodes_to_duplicate
            n_root_dupli = root_dupli_mask.sum().item()
            if n_root_dupli > 0:
                if self.verbose:
                    print(f"  Duplicating {n_root_dupli} root nodes with full subtrees at max_level")
                # Duplicate root nodes (they will create new root nodes at level 1)
                duplicate(
                    params=params,
                    optimizers=optimizers,
                    state=state,
                    mask=root_dupli_mask,
                    levels=levels,
                    parent_indices=parent_indices,
                    level_indices=level_indices,
                )
                # Update levels and parent_indices after duplicate
                levels = state["levels"]
                parent_indices = state["parent_indices"]
                level_indices = state["level_indices"]
                N = len(levels)  # Update N after duplicate
                # Recompute num_child after duplicate (needed for subsequent operations)
                valid_parent_mask = (parent_indices != -1) & (parent_indices >= 0) & (parent_indices < N)
                if valid_parent_mask.any():
                    valid_parent_indices = parent_indices[valid_parent_mask]
                    num_child = torch.bincount(valid_parent_indices, minlength=N)
        ...
        # Note: n_dupli includes root duplications
        return n_dupli + n_root_dupli, n_split, n_create_children, n_root_dupli
