"""
coarse_to_fine_trainer_v3.py - Coarse-to-Fine Multigrid Trainer
...
This trainer implements a coarse-to-fine training approach for hierarchical Gaussian Splatting:
    ...
    - Children are pre-created during initialization (multigrid_gaussians_v4)
    - Densification rules: duplication/split when parent is not full, signal parent when full
    - Uses multigrid_v10 strategy with hierarchy block cloning for efficient densification
"""
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.result_dir is None:
            ...
            self.result_dir = f"/Bean/log/gwangjin/2025/gsplat/coarse_to_fine_v3/{dataset_name}_{settings_str}_v2"
...
class Runner:
    ...
    def __init__(
        ...
        if isinstance(self.cfg.strategy, MultigridStrategy):
            ...
            self.cfg.strategy.reset_every = 12  # Cycle-based: reset every 30 cycles
    ...
    def _clear_grad_scaling_hooks(self):
        for hook in self.grad_scale_hooks:
            hook.remove()
        self.grad_scale_hooks = []
        if hasattr(self, "_grad_scale_factors"):
            del self._grad_scale_factors
        if hasattr(self, "_grad_scale_levels"):
            del self._grad_scale_levels
        torch.cuda.empty_cache()

    def _register_grad_scaling_hooks(self):
        self._clear_grad_scaling_hooks()
        if self.cfg.grad_scale_factor == 1.0:
            return
        ...
        scale_factors = self.cfg.grad_scale_factor ** level_diffs
        self._grad_scale_factors = scale_factors
        self._grad_scale_levels = levels
            ...
            if param.requires_grad:
                def make_hook(scale_factors_tensor):
                    def hook(grad):
                        ...
                        return grad
                    return hook
                self.grad_scale_hooks.append(param.register_hook(make_hook(scale_factors)))
    ...
    def _train_step(
        ...
        visible_mask_cpu = info["visible_mask"]
        if visible_mask_cpu.device.type != "cpu":
            visible_mask_cpu = visible_mask_cpu.to("cpu")
        info_cpu = {
            "width": info["width"],
            "height": info["height"],
            "n_cameras": info["n_cameras"],
            "render_level": info.get("render_level", 1),
            "visible_mask": visible_mask_cpu,
        }
        if "gaussian_ids" in info and info["gaussian_ids"] is not None:
            gaussian_ids_cpu = info["gaussian_ids"]
            if gaussian_ids_cpu.device.type != "cpu":
                gaussian_ids_cpu = gaussian_ids_cpu.to("cpu")
            info_cpu["gaussian_ids"] = gaussian_ids_cpu
        if "radii" in info and info["radii"] is not None:
            radii_cpu = info["radii"].detach()
            if radii_cpu.device.type != "cpu":
                radii_cpu = radii_cpu.to("cpu")
            info_cpu["radii"] = radii_cpu
        if key_for_gradient is not None and key_for_gradient in info:
            grad_tensor = info[key_for_gradient].grad
            if grad_tensor is None:
                grad_cpu = torch.zeros_like(info[key_for_gradient], device="cpu")
            else:
                grad_cpu = grad_tensor.detach().to("cpu")
            grad_proxy = SimpleNamespace(grad=grad_cpu)
            if getattr(self.cfg.strategy, "absgrad", False):
                if hasattr(info[key_for_gradient], "absgrad") and info[key_for_gradient].absgrad is not None:
                    grad_proxy.absgrad = info[key_for_gradient].absgrad.detach().to("cpu")
                else:
                    grad_proxy.absgrad = grad_cpu.abs()
            info_cpu[key_for_gradient] = grad_proxy

        # Turn Gradients into Sparse Tensor before running optimizer
        if cfg.sparse_grad:
            assert cfg.packed, "Sparse gradients only work with packed mode."
            visible_indices = torch.where(info_cpu["visible_mask"])[0]
            gaussian_ids = info_cpu["gaussian_ids"]
    ...
    def train(self):
        ...
        use_global_grad_scaling = cfg.grad_scale_factor != 1.0
        if use_global_grad_scaling:
            self._register_grad_scaling_hooks()
        ...
        for step in pbar:
            ...
            loss = self._train_step(
                ...
                apply_grad_scaling=not use_global_grad_scaling,
            )
            ...
            if cfg.depth_loss:
                del points, depths_gt
            del data, camtoworlds, Ks, pixels, image_ids, masks, num_train_rays_per_step
            torch.cuda.empty_cache()
        ...
        for cam_idx in self.viz_camera_indices:
            ...
            for render_level in levels_to_render:
                ...
                if downsample_factor > 1:
                    ...
                    render_height = int(max(1, height / downsample_factor))
                    render_width = int(max(1, width / downsample_factor))
    ...
    def run_compression(self, step: int):
        ...
        for k in splats_c.keys():
            self.splats[k].data = splats_c[k].to("cpu")
...
def main(local_rank: int, world_rank, world_size: int, cfg: Config):
    ...
    if cfg.ckpt is not None:
        ...
        if "levels" in ckpts[0]:
            runner.multigrid_gaussians.levels = ckpts[0]["levels"].to("cpu")
            runner.multigrid_gaussians.parent_indices = ckpts[0]["parent_indices"].to("cpu")
