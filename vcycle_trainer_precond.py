...
from gsplat.strategy.multigrid_v2 import MultigridStrategy
...
class Config:
    ...
    # Implicit preconditioning parameters
    use_preconditioning: bool = True  # Enable implicit preconditioning
    preconditioning_base: float = 2.0  # Base factor for preconditioning: coarse levels use larger factors
    # Preconditioning factor for level L: base ** (max_level - L)
    # Coarse levels (low L) get larger factors, fine levels (high L) get smaller factors
    num_preconditioning_iter: int = 5  # Number of V-cycle iterations for preconditioning convergence
    ...
    def __post_init__(self):
        ...
        if self.result_dir is None:
            ...
            self.result_dir = f"/Bean/log/gwangjin/2025/gsplat/vcycle/{dataset_name}_{settings_str}"
...
def v_cycle_step_recursive(
    grads: Dict[str, Tensor],
    level: int,
    coarsest_level: int,
    multigrid_gaussians: "MultigridGaussians",
    preconditioning_base: float = 2.0,
) -> Dict[str, Tensor]:
    """
    Recursive V-cycle step for gradient preconditioning.
    
    Structure:
    - Smooth: Apply smoothing to gradients at current level
    - Restrict: Downsample to coarser level
    - Recursive call: Solve at coarser level (or direct solve at coarsest)
    - Prolongate: Upsample correction back to current level
    - Add: correction = prolongate(coarse_correction) + smoothed
    
    Args:
        grads: Dictionary of gradients {param_name: gradient_tensor}
        level: Current level (higher = finer)
        coarsest_level: Coarsest level to recurse to
        multigrid_gaussians: MultigridGaussians instance for level information
        preconditioning_base: Base factor for level-dependent scaling
    
    Returns:
        Preconditioned gradients (correction)
    """
    # Get max level
    ...
    # Smooth: Apply level-dependent smoothing to gradients
    # Coarse levels get larger smoothing (larger step size)
    smoothing_factor = preconditioning_base ** (max_level - level)
    smoothed_grads = {}
    for k, v in grads.items():
        if v is not None:
            smoothed_grads[k] = v * smoothing_factor
        else:
            smoothed_grads[k] = None
    
    # Base case: At coarsest level, return smoothed gradients (direct solve)
    if level <= coarsest_level:
        return smoothed_grads
    
    # Recursive case: Restrict -> Recurse -> Prolongate
    # Restrict: Get gradients for gaussians at next coarser level
    # For simplicity, we'll use level-based filtering
    # Get gaussians at current level and next coarser level
    current_level_mask = (multigrid_gaussians.levels == level)
    coarser_level_mask = (multigrid_gaussians.levels == level - 1)
    
    # Restrict: Average gradients for gaussians that share the same parent
    # For now, we'll use a simplified approach: filter by level
    restricted_grads = {}
    for param_name, grad in smoothed_grads.items():
        if grad is None:
            restricted_grads[param_name] = None
            continue
        
        # Get gradients for coarser level gaussians
        if coarser_level_mask.any():
            coarser_grad = grad[coarser_level_mask]
        else:
            # No coarser level gaussians, use current level
            coarser_grad = grad[current_level_mask] if current_level_mask.any() else grad
        
        restricted_grads[param_name] = coarser_grad
    
    # Recursive call: Solve at coarser level
    coarse_correction = v_cycle_step_recursive(
        grads=restricted_grads,
        level=level - 1,
        coarsest_level=coarsest_level,
        multigrid_gaussians=multigrid_gaussians,
        preconditioning_base=preconditioning_base,
    )
    
    # Prolongate: Map coarse correction back to fine level
    # For simplicity, we'll use parent-child relationships
    # Each fine gaussian inherits from its parent (coarse gaussian)
    prolonged_grads = {}
    for param_name, coarse_corr in coarse_correction.items():
        if coarse_corr is None:
            prolonged_grads[param_name] = None
            continue
        
        # Initialize with smoothed gradients
        fine_corr = smoothed_grads[param_name].clone()
        
        # Add prolonged coarse correction
        # For gaussians at current level, add parent's correction
        if level > 1:  # Not at root level
            parent_indices = multigrid_gaussians.parent_indices
            current_level_indices = torch.where(current_level_mask)[0]
            
            if len(current_level_indices) > 0:
                current_parents = parent_indices[current_level_indices]
                valid_parent_mask = (current_parents >= 0) & (current_parents < len(coarse_corr))
                
                if valid_parent_mask.any():
                    valid_indices = current_level_indices[valid_parent_mask]
                    valid_parents = current_parents[valid_parent_mask]
                    
                    # Add parent's correction to child
                    if len(coarse_corr.shape) > 1:
                        # Multi-dimensional: [N, ...]
                        fine_corr[valid_indices] = fine_corr[valid_indices] + coarse_corr[valid_parents]
                    else:
                        # 1D: [N]
                        fine_corr[valid_indices] = fine_corr[valid_indices] + coarse_corr[valid_parents]
        
        prolonged_grads[param_name] = fine_corr
    
    return prolonged_grads


def v_cycle_preconditioner_implicit(
    raw_grads: Dict[str, Tensor],
    multigrid_gaussians: "MultigridGaussians",
    coarsest_level: int = 1,
    num_forward_iter: int = 5,
    preconditioning_base: float = 2.0,
) -> Dict[str, Tensor]:
    """
    Implicit differentiation with V-cycle preconditioning.
    
    Forward: V-cycle 반복 (no_grad) - 수렴할 때까지 반복
    Backward: Fixed point에서 implicit diff (메모리 O(1))
    
    Args:
        raw_grads: Dictionary of raw gradients {param_name: gradient_tensor}
        multigrid_gaussians: MultigridGaussians instance
        coarsest_level: Coarsest level for V-cycle
        num_forward_iter: Number of V-cycle iterations for convergence
        preconditioning_base: Base factor for level-dependent scaling
    ...
    # ===== Forward: 빠르게 수렴 =====
    with torch.no_grad():
        x = {k: v.clone() if v is not None else None for k, v in raw_grads.items()}
        ...
        # V-cycle을 여러 번 반복 (convergence)
        for _ in range(num_forward_iter):
            x = v_cycle_step_recursive(
                grads=x,
                level=max_level,  # Start from max level
                coarsest_level=coarsest_level,
                multigrid_gaussians=multigrid_gaussians,
                preconditioning_base=preconditioning_base,
            )
    ...
    # Fixed point 조건: x* = F(x*, raw_grads)
    # 우리가 원하는 것: ∂x*/∂raw_grads
    
    # For implicit differentiation, we need to compute the Jacobian of the fixed point
    # However, for efficiency, we'll use a simpler approach:
    # The preconditioned gradient is the fixed point itself (after convergence)
    # This is equivalent to implicit differentiation when the fixed point iteration converges
    
    # Return the final correction as preconditioned gradients
    # This is the result of V-cycle preconditioning
...
def perform_vcycle(
    ...
    """
    Perform a single V-cycle: max_level -> coarsest_level -> max_level (recursive).
    
    Recursive V-cycle structure:
    - Downward: Smooth at current level, then recurse to coarser level
    - Coarsest: Direct solve
    - Upward: Prolongate correction from coarser level, add to smoothed
    ...
    def _vcycle_recursive(
        ...
        """Recursive V-cycle helper."""
        # Base case: coarsest level - solve
        if level <= coarsest:
            local_steps = solving_steps * (cfg.steps_decaying_per_level ** (max_level - coarsest))
            local_steps = max(1, int(local_steps))
            for _ in range(local_steps):
                if current >= total_steps:
                    return current, losses_list
                ...
                log_step = min(current, total_steps - 1) if total_steps > 0 else current
                loss = runner._train_step(step=log_step, data=data, target_level=level)
                ...
                if pbar is not None:
                    ...
                    desc = f"V-cycle {vcycle_idx}| Solve L{level}| loss={avg_loss:.3f}| step={current}/{total_steps}"
                    pbar.set_description(desc)
            
            return current, losses_list
        
        # Recursive case: Downward -> Recurse -> Upward
        
        # Downward: Smooth at current level
        local_smoothing = smoothing_steps * (cfg.steps_decaying_per_level ** (max_level - level))
        local_smoothing = max(1, int(local_smoothing))
        for _ in range(local_smoothing):
            if current >= total_steps:
                return current, losses_list
            ...
            if pbar is not None:
                pbar.update(1)
                avg_loss = np.mean(losses_list) if losses_list else 0.0
                desc = f"V-cycle {vcycle_idx}| Down L{level}| loss={avg_loss:.3f}| step={current}/{total_steps}"
                pbar.set_description(desc)
        
        # Recurse to coarser level
        current, losses_list = _vcycle_recursive(level - 1, coarsest, current, losses_list)
        
        # Upward: Smooth at current level (after prolongation)
        for _ in range(local_smoothing):
            if current >= total_steps:
                return current, losses_list
            ...
            if pbar is not None:
                pbar.update(1)
                avg_loss = np.mean(losses_list) if losses_list else 0.0
                desc = f"V-cycle {vcycle_idx}| Up L{level}| loss={avg_loss:.3f}| step={current}/{total_steps}"
                pbar.set_description(desc)
        
        return current, losses_list
    
    # Ensure coarsest_level is valid
    coarsest_level = max(1, min(coarsest_level, max_level))
    
    # Start recursive V-cycle from max_level
    losses = []
    current_step, losses = _vcycle_recursive(max_level, coarsest_level, current_step, losses)
...
class Runner:
    ...
    def _train_step(
        ...
        # Apply implicit V-cycle preconditioning
        if cfg.use_preconditioning:
            ...
            for param_name, param in self.splats.items():
                if param.grad is not None and not param.grad.is_sparse:
                    raw_grads[param_name] = param.grad
                else:
                    raw_grads[param_name] = None
            
            # Get coarsest level for preconditioning
            ...
            # Apply V-cycle preconditioner with implicit differentiation
            precond_grads = v_cycle_preconditioner_implicit(
                raw_grads=raw_grads,
                multigrid_gaussians=self.multigrid_gaussians,
                coarsest_level=coarsest_level,
                num_forward_iter=cfg.num_preconditioning_iter,
                preconditioning_base=cfg.preconditioning_base,
            )
            
            # Replace gradients with preconditioned gradients
            for param_name, param in self.splats.items():
                if param.grad is not None and not param.grad.is_sparse and param_name in precond_grads:
                    if precond_grads[param_name] is not None:
                        param.grad = precond_grads[param_name]

        # Turn Gradients into Sparse Tensor before running optimizer
        # Note: Preconditioning is already applied above (before sparse conversion)
        if cfg.sparse_grad:
    ...
    def save_visualization(self, step: int):
        ...
        Saves level-wise renderings and diff maps separately for each level.
        For each level, saves:
        - render_level_N_cam_M.png: Level N rendering for camera M
        - diff_level_N_vs_N-1_cam_M.png: Difference between Level N and Level N-1
        - diff_level_N_vs_GT_cam_M.png: Difference between Level N and GT
        """
        ...
        for cam_idx in self.viz_camera_indices:
            ...
            for render_level in levels_to_render:
                ...
                render_path = os.path.join(viz_dir, f"render_level_{render_level}_cam_{cam_idx:04d}_step_{step:06d}.png")
                render_uint8 = (colors_np * 255).astype(np.uint8)
                imageio.imwrite(render_path, render_uint8)
                
                # Calculate and save diff vs GT
                diff_vs_gt = np.abs(colors_np - pixels_gt.cpu().numpy())  # [H, W, 3]
                ...
                diff_gt_path = os.path.join(viz_dir, f"diff_level_{render_level}_vs_GT_cam_{cam_idx:04d}_step_{step:06d}.png")
                ...
                if prev_colors_np is not None:
                    diff_vs_prev = np.abs(colors_np - prev_colors_np)  # [H, W, 3]
                    # Normalize to [0, 1] and apply colormap-like visualization
                    diff_vs_prev_vis = np.clip(diff_vs_prev * 3.0, 0.0, 1.0)
                    diff_prev_path = os.path.join(viz_dir, f"diff_level_{render_level}_vs_{render_level-1}_cam_{cam_idx:04d}_step_{step:06d}.png")
                ...
                prev_colors_np = colors_np.copy()
        
        print(f"  Saved level-wise visualizations to {viz_dir} (levels: {levels_to_render}, cameras: {self.viz_camera_indices})")
