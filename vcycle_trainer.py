...
def perform_vcycle(
    ...
    # From max_level to coarsest_level
    # Skip downward pass if max_level == coarsest_level (already at coarsest level)
    if max_level > coarsest_level:
        for level in range(max_level, coarsest_level, -1):
            target_level = level
            
            # Smoothing steps at current level
            local_smoothing_steps = smoothing_steps * (cfg.steps_decaying_per_level ** (max_level - level))
            local_smoothing_steps = max(1, int(local_smoothing_steps))
            for smoothing_step in range(local_smoothing_steps):
    ...
    target_level = coarsest_level
    local_solving_steps = smoothing_steps * (cfg.steps_decaying_per_level ** (max_level - coarsest_level))
    local_solving_steps = max(1, int(local_solving_steps))
    ...
    # From coarsest_level+1 to max_level
    # Skip upward pass if max_level == coarsest_level (no finer levels to go to)
    if max_level > coarsest_level:
        for level in range(coarsest_level + 1, max_level + 1):
            target_level = level
            
            # Smoothing steps at current level
            local_smoothing_steps = smoothing_steps * (cfg.steps_decaying_per_level ** (level - coarsest_level))
            local_smoothing_steps = max(1, int(local_smoothing_steps))
            for smoothing_step in range(local_smoothing_steps):
                # Check if we've exceeded total_steps before processing this step
                if current_step >= total_steps:
